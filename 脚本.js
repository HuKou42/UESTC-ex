...
const 地址变图像元素 = (地址,回调)=>{
    const 图 = 新建图();
    添加事件监控(图,加载,_=>设置延时(_=>回调(图),1200 / 二));
    图[源] = 地址;
};
...
const 保存图像 = _=>{
    ...
    const 文档文本 = `<?xml version="1.0" encoding="utf-8"?><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -50 1200 1200" ${宽度}="1200px" ${高度}="1200px">${图形[肉]}</svg>`;
